...
function processRawData(rawData) {
    ...
    var currentElement = select(rootElement).select('svg').remove();
    var svg = select(rootElement)
        .append('svg')
        .attr('width', ww)
        .attr('height', hh)
        .append('g');
...
function panel(margin, width, linechartmargin, all, warning, refheight, lineitem, svg) {
    var sideScale = linechartmargin.sideScale, keepleft = linechartmargin.left, formatC = format('.2f'), triangledown = function (ll) {
        return "\n        M0 0\n        l" + -ll / 3 + "  " + -ll + "\n        l" + 2 * ll / 3 + " 0\n        z";
    ...
    var x = linear$2().range([0, width - linechartmargin.left - linechartmargin.right])
    ...
    if (all) {
        ...
        if (!warning) {
            changestyle = linechart.append('text').attr('class', 'sidetext')
                ...
                select(event.currentTarget).attr('id', 'panel6').attr('dx', -10).transition().duration(1000).attr('dx', 0).attr('id', "" + (good_1 ? 'panel3' : 'panel1'));
                //callback(lineitem);
            });
            if (sideScale != 1) {
                console.log(changestyle.attr('class'));
            ...
            changestyle = linechart.append('path').attr('d', function () {
                ...
                select(event.currentTarget).attr('id', 'panel6').attr('transform', "translate(" + (flagstart + linechartmargin.side - 10) + "," + -linechartmargin.side * 2 + ")")
